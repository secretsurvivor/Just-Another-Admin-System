...
function SQLTableObject:Insert()
	error("Function not implemented", 2)
end

function SQLTableObject:Update()
	error("Function not implemented", 2)
end

function SQLTableObject:Delete()
	error("Function not implemented", 2)
end
...
if SERVER then
	if !file.Exists("jaas/logs/", "DATA") then
	...
	function jaas_log:WriteToLog(record)
		/* type [Usage] - Opcode - Description
			...
			String* O - 0x8 - Structure
				...
				String*
		*/
		local file_name = "jaas/logs/" + os.date("%Y%m%d") + ".dat"
		local f
	...
	function log_record:NetWrite()
		...
		net.WriteUInt(#self.Rank, 16) -- Rank
		if #self.Rank > 0 then
		...
		net.WriteUInt(#self.Player, 16) -- Player
		...
		net.WriteUInt(#self.Entity, 16) -- Entity
		...
		net.WriteUInt(#self.Data, 16) -- Data
		if #self.Data > 0 then
		...
		net.WriteUInt(#self.String, 16) -- String
	...
	function log_record:NetRead()
		...
		local amount = net.ReadUInt(16) -- Rank Amount
		local i = 1
		if amount > 0 then
			repeat
		...
		amount = net.ReadUInt(16) -- Player Amount
		if amount > 0 then
			i = 1
			repeat
		...
		amount = net.ReadUInt(16) -- Entity Amount
		if amount > 0 then
			i = 1
			repeat
		...
		amount = net.ReadUInt(16) -- Data Amount
		if amount > 0 then
			i = 1
			repeat
		...
		amount = net.ReadUInt(16) -- String Amount
		if amount > 0 then
			i = 1
			repeat
...
function JAAS:NewModule(module_name)
	...
	module_list[1 + #module_list] = setmetatable({}, {__index = jaas_module})
	...
	for k,module_data in ipairs(module_list) do -- k = index, v = module data
		if module_dependencies[module_data.name] == nil then
			...
			for k,v in ipairs(module_dependencies[module_data.name]) do -- name,module_name
				module_list[v].Shared["Post" + module_data.name](module_data)
			end
			...
			if SERVER then
				...
				for k,v in ipairs(module_dependencies[module_data.name]) do -- name,module_name
					module_list[v].Server["Post" + module_data.name](module_data)
				end
			end
			...
			if CLIENT then
				...
				for k,v in ipairs(module_dependencies[module_data.name]) do -- name,module_name
					module_list[v].Client["Post" + module_data.name](module_data)
				end
			end
		end
	end
...
local CLIENTLOG = jaas_net:RegisterNetworkString("Base::ModuleClientLog")

if SERVER then
	function jaas_module:ClientPrint(ply, str)
		jaas_net:SendString(CLIENTLOG, "JAAS::" + self.name + ":: " + str, ply)
	end
elseif CLIENT then
	function jaas_module:ClientPrint(ply, str) end -- To avoid errors caused by use in Shared modules

	jaas_net:ReceiveString(CLIENTLOG, function (str)
		print(str)
	end)
end
...
jaas_module = SQLTableObject:Inherit(jaas_module, "Module")

JAAS.Module = jaas_module